...
impl CommandHandler {
    ...
    pub fn handle_init(&mut self) -> Result<()> {
        ...
        println!("✓ Commandy initialized successfully");
        Ok(())
    }
    
    pub fn handle_config(&self) -> Result<()> {
        println!("Commandy Configuration:");
...
impl OllamaClient {
    ...
    fn build_prompt(&self, user_prompt: &str, context: &ContextData) -> Result<String> {
        ...
        // recent commands, and COMMANDY.md patterns for enhanced suggestions
    }
...
impl ContextManager {
    pub fn new(settings: &Settings) -> Result<Self> {
        ...
        let cache_path = storage.get_commandy_dir().join("cache/suggestions.db");
    ...
    pub fn get_relevant_context(&self, prompt: &str) -> Result<ContextData> {
        ...
        if let Ok(shell_history) = self.cache.get_shell_history() {
            // Merge relevant shell commands with commandy history
        }
    ...
    pub fn record_command_execution(
        ...
        // Update COMMANDY.md with successful patterns
        if success {
    ...
    match cli.command {
        ...
        None => {
            if let Some(ref prompt) = cli.prompt {
                ...
                // Show help message
                println!("Commandy - Secure, fast command suggestions using local models");
                // ... help text
            }
        }
    }
...
license = "MIT"
repository = "https://github.com/aptro/commandy"
