...
async fn main() -> Result<()> {
    ...
    let mut handler = match CommandHandler::new() {
        Ok(h) => h,
        Err(e) => {
            error!("Failed to initialize Commandy: {e}");
            eprintln!("Error: Failed to initialize Commandy: {e}");
            eprintln!("Try running 'commandy init' first.");
            std::process::exit(1);
        }
    };
    ...
    match cli.command {
        ...
        None => {
            if let Some(ref prompt) = cli.prompt {
                ...
                let help = r#"Commandy - Secure, fast command suggestions using local models

Usage:
  commandy [OPTIONS] <PROMPT>
  commandy [COMMAND]

Examples:
  commandy "list running containers"
  commandy "find large files in current directory"
  commandy --explain "git commit with message"
...
For more information, visit: https://commandy.sh
"#;
                println!("{help}");
            }
        }
